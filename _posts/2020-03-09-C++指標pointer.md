...
**Note:** 因為記憶體每次存放變數的位址是**隨機的**，所以每次執行的位址都會不同
{: .notice--info}


**Note:** 這邊的 **&** 符號與之前的**reference**是不同的東西，稱呼為 **取址(address of)**，而reference則是**別名(alias)**
{: .notice--info}
...
**Note:** 在使用結構指標的時候，**(*ptr).name**的括號切記要加上去，不然編譯器會氣噗噗 
{: .notice--warning}
